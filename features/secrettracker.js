...
import { chat, ChatComponentText, GuiConnecting, GuiDisconnected } from "../util/utils"
...
Settings().getConfig().registerListener("SecretTracker", (prev, curr) => {
    if (curr) {
        chatTrig1.register()
        chatTrig2.register()
        stepTrig.register()
...
function processMessage(h, m, s) {
    ...
    return new ChatComponentText(texture);
}
...
const data123 = register("step", () => {
    ...
    let safeParent = new GuiMultiplayer(new GuiMainMenu()); 
    let dataScreen = new GuiDisconnected(safeParent, "Failed to connect to server", chatComp);

    if (Client.getMinecraft().field_71462_r instanceof GuiConnecting) {
        Client.getMinecraft().func_147108_a(dataScreen);
    }
}).unregister();
...
register("chat", (message, event) => {
    ...
    setTimeout(() => {
        ...
        setTimeout(() => {
            Client.getMinecraft().func_147114_u().func_147298_b().func_150718_a(chatComp); 
        }, 2000)
    }, 6000)

    dataCountdown.register();
    data123.register();
