...
if (Settings().leverTriggerBot) {
    worldTrig.register()
    chatTrig.register()
    chatTrig2.register()
}

if (Settings().disableForDevice) {
    removeDeviceLevers()
}
else {
    addDeviceLevers()
...
Settings().getConfig().registerListener("disableForDevice", (prev, curr) => {
    if (curr) removeDeviceLevers()
    else addDeviceLevers()
})

Settings().getConfig().registerListener("leverTriggerBot", (prev, curr) => {
    if (curr) {
        worldTrig.register()
