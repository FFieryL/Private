...
registerOverlay("MelodyTitle", { text: () => "&cFiery (Berserk) &ehas Melody! 1/4", align: "center", colors: false})
...
const chatTrig = register("chat", (message) => {
    ...
    if (!currentProgress) {
        ...
        World.playSound("random.orb", 1, 0.8);
        return;
    }
...
const overlayTrig = register("renderOverlay", () => {
    ...
    drawText(displayText, data.MelodyTitle, true, "MelodyTitle")
}).unregister()
...
Settings().getConfig().registerListener("MelodyTitle", (prev, curr) => {
    if (curr) {
        resetMelody()
        stepTrig.register()
        chatTrig.register()
